...
class ComputeOmegas:
    ...
    def compute_omega(
        ...
        for (l,m,n) in lmn_modes:
            df = self.create_a_over_M_omegas_dataframe(l,m,n,file_path)
            mode = f'({l},{m},{n})'
        ...
        if not os.path.exists('data' + file_path[-4:] + '/omegas'):
            os.makedirs('data' + file_path[-4:] + '/omegas')

        with open(f'data{file_path[-4:]}/omegas/Mf_qnm_freqs_{label}.json', 'w') as fp:
            json.dump(omegas, fp, sort_keys=True, indent=4)

        with open(f'data{file_path[-4:]}/omegas/Mi_qnm_freqs_{label}.json', 'w') as fp:
    ...
    def create_a_over_M_omegas_dataframe(
        ...
        if m<0:
            m = f'm{abs(m)}'
        files = np.genfromtxt(f'{file_path}/frequencies_l{l}/n{n+1}l{l}m{m}.dat', usecols=range(3))
