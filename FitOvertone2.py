...
class FitOvertone:
    ...
    def __init__(
        ...
        modes = (f'({lm_fit[0]},{lm_fit[1]},0)', f'({lm_fit[0]},{lm_fit[1]},1)', f'({lm_fit[0]},{lm_fit[1]},2)')
        ...
        fit_dtheta.append(
            lmfit.minimize(
                ...
                kws={
                    ...
                    'wr2': omegas[modes[2]]['omega_r'],
                    'wi0': omegas[modes[0]]['omega_i'],
                    'wi1': omegas[modes[1]]['omega_i'],
                    'wi2': omegas[modes[2]]['omega_i'],
    ...
    def compute_fit_strain(self, data_strain, omegas, modes):
        ...
        params_strain.add('A2r', value=0.01, vary=True)
    ...
    def save_strain_fit(self, data_strain, fit_strain):
        ...
        for i in range(len(fit_strain)):
            save_strain.append([
                ...
                fit_strain[i].params['A2r'].value,
                fit_strain[i].params['t0i'].value,
                fit_strain[i].params['t1i'].value,
                fit_strain[i].params['t2i'].value,
                (fit_strain[i].params['A1r'].value/
                    fit_strain[i].params['A0r'].value),
                (fit_strain[i].params['A2r'].value/
                    fit_strain[i].params['A0r'].value),
    ...
    def _compute_min_mismatch(
        ...
        columns_fit = {
            'dtheta': ['time', 'R', 'dphi'],
            'strain': ['time', 'A_0', 'A_1', 'A_2', 'phi_0', 'phi_1', 'phi_2', 'R', 'dphi']
            }
...
class FitFunctions:
    ...
    # fundamental mode + 2nd overtone strain
    def f_Re_3_modes(self, x, A0, A1, A2, t0, t1, t3, wr0, wr1, wr2, wi0, wi1, wi2):
        return \
            A0*exp(-wi0*x)*cos(wr0*x-t0) + \
            A1*exp(-wi1*x)*cos(wr1*x-t1)

    def f_Im_3_modes(self, x, A0, A1, A2, t0, t1, t3, wr0, wr1, wr2, wi0, wi1, wi2):
        return \
            A0*exp(-wi0*x)*sin(wr0*x-t0) + \
            A1*exp(-wi1*x)*sin(wr1*x-t1)

    def residual_3_modes(self, params, wr0=None, wr1=None, wr2=None, wi0=None, wi1=None, wi2=None, x=None, datRe=None, datIm=None):
        modelRe = self.f_Re_2_modes(x, params['A0r'], params['A1r'],params['A2r'], params['t0r'], params['t1r'], wr0, wr1, wi0, wi1)
        modelIm = self.f_Im_2_modes(x, params['A0i'], params['A1i'], params['A2i'], params['t0i'], params['t1i'], wr0, wr1, wi0, wi1)
