...
class SaveParameters:
    ...
    def __init__(
        ...
        if not os.path.exists(f'data{file_path[-4:]}/qnm_pars'):
            os.makedirs(f'data{file_path[-4:]}/qnm_pars')

        harm_fits = self.import_harmonics(lm_modes, file_path)
        harm_fits = self.compute_ratios_dphis(harm_fits, lm_dominant)

        over_fits = self.import_overtone_fit(lm_dominant, file_path)
        over_fits = self.compute_A_phase_dtheta(harm_fits, over_fits, lm_dominant)

        self.import_simulation(file_path)

        self.save_pars(harm_fits,over_fits, file_path)
        ...
        with open(f'data{file_path[-4:]}/qnm_pars/omegas.json', 'w') as fp:
    ...
    def save_pars(
        ...
        with open(f'data{file_path[-4:]}/qnm_pars/ratios.json', 'w') as fp:
            json.dump(ratios, fp, sort_keys=True, indent=4)

        with open(f'data{file_path[-4:]}/qnm_pars/dphi.json', 'w') as fp:
            json.dump(dphis, fp, sort_keys=True, indent=4)

        with open(f'data{file_path[-4:]}/qnm_pars/amplitudes.json', 'w') as fp:
            json.dump(amplitudes, fp, sort_keys=True, indent=4)

        with open(f'data{file_path[-4:]}/qnm_pars/phases.json', 'w') as fp:
    ...
    def import_harmonics(
        ...
        harm_fits = {}
        for (l,m) in lm_modes:
            with open(f'data{file_path[-4:]}/fits/best_fit_t10_l{l}m{m}.json') as f:
    ...
    def import_overtone_fit(
        ...
        (l, m) = lm_dominant
        over_fits = {}
        with open(f'data{file_path[-4:]}/overtone_fits/best_fit_dtheta_t10.json') as f:
            over_fits[f'({l},{m},1) I'] = json.load(f)

        with open(f'data{file_path[-4:]}/overtone_fits/best_fit_strain_t10.json') as f:
    ...
    def import_simulation(
        ...
        with open(f'data{file_path[-4:]}/qnm_pars/bh_pars.json', 'w') as fp:
            json.dump({
