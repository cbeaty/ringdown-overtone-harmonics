...
class FundamentalFit():
    ...
    def fit_fundamental_mode(
        ...
        t_initial, t_final = self._find_times_fundamental(file_path, lm_dominant, lm_dominant)
        
        # create folder data/fits
        if not os.path.exists(f'data{file_path[-4:]}/fits'):
            os.makedirs(f'data{file_path[-4:]}/fits')

        # import simulation frequencies
        with open(f'data{file_path[-4:]}/omegas/Mi_qnm_freqs_sim.json') as file:
        ...
        for (l,m) in lm_modes:
            key = f'l{l}m{m}'
            data_peak = np.genfromtxt(f'data{file_path[-4:]}/waveforms/peak_{label_dominant}_{key}.dat', delimiter='\t')
            ...
            fitfile = open(f'data{file_path[-4:]}/fits/fit_results_fundamental_{key}.txt', 'w')
            sys.stdout = fitfile
            ...
            self.plot_fit(data_peak, key, fit, t_initial, t_final, file_path)
    ...
    def fit_fundamental_mode_amplitude(
        ...
        for (l,m) in lm_modes:
            ...
            # save fitted parameters to file
            orig_stdout = sys.stdout
            fitfile = open(f'data{file_path[-4:]}/fits/fit_results_fundamental_amplitudes_{key}.txt', 'w')
            sys.stdout = fitfile
            print(lmfit.fit_report(fit))
            sys.stdout = orig_stdout
            fitfile.close()
            ...
            with open(f'data{file_path[-4:]}/fits/fit_amp_phase_fundamental_{key}.dat', 'w') as f:
                np.savetxt(
                    f,
                    [
                        data['time'].values[0],
            ...
            with open(f'data{file_path[-4:]}/fits/best_fit_t10_{key}.json', 'w') as fp:
                json.dump({
                    'time': 10,
            ...
            self.plot_fit(data_peak, key+"_omega_sim", fit, t_initial, t_final, file_path)
    ...
    def _find_times_fundamental(
        ...
        d_arctan = np.genfromtxt(f'data{file_path[-4:]}/d_theta/peak_{label_d}_{label_t}.dat', delimiter='\t')
        const = []
        ...
        np.savetxt(f'data{file_path[-4:]}/times_fundamental.dat', (t_initial, t_final), delimiter='\t', header = f'#(0) initial (1) final # ({label_t[0]},{label_t[1]},0) mode')
    ...
    def plot_fit(self, data, label_mode, fit, t_initial, t_final, file_path):
        # plot fitted strain
        font_size = 30
        lw = 3
        ...
        fig.savefig(f'figs{file_path[-4:]}/waveforms_fit_{label_mode}.pdf', bbox_inches="tight")
