...
def plot_mismatch(mismatch, mins, file_path):
...
def find_best_fit(mins, fit_pars, file_path):
    best_pars = {
...
def plot_fitted_expected(fit_pars, best_pars, exp_pars, t_fin, file_path):
...
def plot_best_fits(best_pars, data_dtheta, data_strain, omegas, modes, t_initial, file_path):
    # plot best fit
...
def main(omegas, file_path):
    
    #import data
    t_initial = np.genfromtxt(f'data{file_path[-4:]}/times_fundamental.dat')[0]

    data_strain = np.genfromtxt(f'data{file_path[-4:]}/waveforms/peak_22_l2m2.dat', delimiter='\t')
    data_dtheta = np.genfromtxt(f'data{file_path[-4:]}/d_theta/peak_22_l2m2.dat', delimiter='\t')
    ...
    for key in ['dtheta', 'energy_dtheta', 'strain', 'energy_strain']:
        ...
        try:
            fit_pars[key] = np.genfromtxt(f'data{file_path[-4:]}/overtone_fits/fitpar_{key}.dat')
    ...
    # plot mismatch
    plot_mismatch(mismatch, mins, file_path)

    best_pars = find_best_fit(mins, fit_pars, file_path)
    exp_pars = {}
    exp_pars['dtheta'] = find_pars_time_dtheta(data_dtheta, best_pars, omegas, file_path)
    exp_pars['strain'] = find_pars_time_strain(data_strain, best_pars, omegas, file_path)

    for (key, value) in exp_pars.items():
        with open(f'data{file_path[-4:]}/overtone_fits/best_fit_{key}_t10.json', 'w') as fp:
    ...
    plot_fitted_expected(fit_pars, best_pars, exp_pars, max(initial_times)[0] + 10, file_path)
    plot_best_fits(best_pars, data_dtheta, data_strain, omegas, modes, t_initial, file_path)
