...
class FitOvertone:
    ...
    def __init__(
        ...
        if not os.path.exists('data'+ file_path[-4:] +'/overtone_fits'):
            os.makedirs('data'+ file_path[-4:] +'/overtone_fits')

        label_fit = f'l{lm_fit[0]}m{lm_fit[1]}'
        label_dominant = f'l{lm_dominant[0]}m{lm_dominant[1]}'

        data_dtheta, data_strain = self._import_data(label_fit, label_dominant, file_path)
        ...
        self.save_dtheta_fit(data_dtheta, fit_dtheta, file_path)
        fit_strain = self.compute_fit_strain(data_strain, omegas, modes)
        self.save_strain_fit(data_strain, fit_strain, file_path)

        mis_dtheta = self.compute_mismatch_dtheta(data_dtheta, fit_dtheta, omegas, modes, file_path)
        mis_strain = self.compute_mismatch_strain(data_strain, fit_strain, omegas, modes, file_path)
        mismatch = {**mis_dtheta, **mis_strain}
        
        mins, fit_pars = self._compute_min_mismatch(mismatch)

        self.plots.plot_mismatch(mismatch, mins, file_path)

        best_pars = self.find_best_fit(mins, fit_pars, file_path)

        exp_pars = {}
        exp_pars['dtheta'] = self.find_pars_time_dtheta(data_dtheta, best_pars, omegas, file_path)
        exp_pars['strain'] = self.find_pars_time_strain(data_strain, best_pars, omegas, file_path)
        ...
        self.plots.plot_fitted_expected(fit_pars, best_pars, exp_pars, max(initial_times)[0] + 10, file_path)
        
        t_initial = np.genfromtxt('data'+file_path[-4:] +'/times_fundamental.dat')[0]
        self.plots.plot_best_fits(best_pars, data_dtheta, data_strain, omegas, modes, t_initial, file_path)
    ...
    def _import_data(
        ...
        # import final time
        t_final = np.genfromtxt('data'+file_path[-4:] +'/times_fundamental.dat')[1]

        # import data
        data_strain = np.genfromtxt(f'data{file_path[-4:]}/waveforms/peak_{label_dominant}_{label_fit}.dat', delimiter='\t')
        data_dtheta = np.genfromtxt(f'data{file_path[-4:]}/d_theta/peak_{label_dominant}_{label_fit}.dat', delimiter='\t')
    ...
    def save_dtheta_fit(self, data_dtheta, fit_dtheta, file_path):
        save_dtheta = []
        ...
        with open('data'+file_path[-4:] +'/overtone_fits/fitpar_dtheta.dat', 'w') as f:
    ...
    def save_strain_fit(self, data_strain, fit_strain, file_path):
        save_strain = []
        ...
        with open('data'+file_path[-4:] +'/overtone_fits/fitpar_strain.dat', 'w') as f:
            np.savetxt(f, save_strain,  delimiter='\t', header = '#(1)time (2)A_0 (3)A_1 (4)phi_0 (5)phi_1 (6)R (7)dphi')

    def compute_mismatch_strain(self, data_strain, fit_strain, omegas, modes, file_path):
        ...
        with open('data'+file_path[-4:] +'/overtone_fits/mismatch_strain.dat', 'w') as f:
            np.savetxt(
                f,
                np.column_stack((
        ...
        with open('data'+file_path[-4:] +'/overtone_fits/mismatch_energy_strain.dat', 'w') as f:
            np.savetxt(
                f,
                np.column_stack((
    ...
    def compute_mismatch_dtheta(self, data_dtheta, fit_dtheta, omegas, modes, file_path):
        ...
        with open('data'+file_path[-4:] +'/overtone_fits/mismatch_dtheta.dat', 'w') as f:
            np.savetxt(
                f,
                np.column_stack((
        ...
        with open('data'+file_path[-4:] +'/overtone_fits/mismatch_energy_dtheta.dat', 'w') as f:
            np.savetxt(
                f,
                np.column_stack((
    ...
    def find_best_fit(self, mins, fit_pars, file_path):
        best_pars = {
        ...
        for (key, value) in best_pars.items():
            with open(f'data{file_path[-4:]}/overtone_fits/best_fit_{key}.json', 'w') as fp:
                json.dump(value.to_dict(), fp, indent=4)
        return best_pars 

    def find_pars_time_dtheta(self, data_dtheta, best_pars, omegas, file_path):
        ...
        with open(f'data{file_path[-4:]}/overtone_fits/best_fit_dtheta_t10.json', 'w') as fp:
            json.dump({
                'time': 10,
    ...
    def find_pars_time_strain(self, data_strain, best_pars, omegas, file_path):
        ...
        with open(f'data{file_path[-4:]}/overtone_fits/best_fit_strain_t10.json', 'w') as fp:
            json.dump({
                'time': 10,
                'R': A1_10/A0_10,
...
class Plots:
    ...
    def plot_mismatch(self, mismatch, mins, file_path):
        ...
        plt.savefig(f'figs{file_path[-4:]}/mismatch.pdf', bbox_inches="tight")

    def plot_fitted_expected(self, fit_pars, best_pars, exp_pars, t_fin, file_path):
        ...
        fig.tight_layout()
        fig.savefig(f'figs{file_path[-4:]}/fit_pars.pdf', bbox_inches="tight")

    def plot_best_fits(self, best_pars, data_dtheta, data_strain, omegas, modes, t_initial, file_path):
        # plot best fit
        ...
        plt.subplots_adjust(hspace=.001)
        fig.tight_layout()
        fig.savefig(f'figs{file_path[-4:]}/best_fits.pdf', bbox_inches="tight")
