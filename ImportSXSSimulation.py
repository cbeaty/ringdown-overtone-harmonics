...
class ImportSimulation():
    ...
    def compute_d_theta(
        ...
        if not os.path.exists(f'data{file_path[-4:]}/d_theta'):
            os.makedirs(f'data{file_path[-4:]}/d_theta')

        open(f'data{file_path[-4:]}/d_theta/{label_data}_{label_lm}.dat', 'w').close()  # clear file
        np.savetxt(
            f'data{file_path[-4:]}/d_theta/{label_data}_{label_lm}.dat',
            d_arctan,
            delimiter='\t',
    ...
    def save_par_simu_to_files(
        ...
        # create folder data/waveforms
        if not os.path.exists(f'data{file_path[-4:]}/waveforms'):
            os.makedirs(f'data{file_path[-4:]}/waveforms')

        # save to file
        header = '# (1) time \t (2) real part \t(3) imaginary part'
        for key, value in files_dic.items():
            np.savetxt('data'+file_path[-4:]+'/waveforms/all_{}_sim.dat'.format(key), value, delimiter='\t', header = header)
        ...
        for key, value in  files_dic.items():
            ...
            # save to file
            np.savetxt(f'data{file_path[-4:]}/waveforms/peak_{key}.dat', peak_strain[key], delimiter='\t', header=header)
            np.savetxt(f'data{file_path[-4:]}/waveforms/peak_{key_dominant}_{key}.dat', peak_dominant_strain[key], delimiter='\t', header=header)
            ...
            self.compute_d_theta(peak_strain[key], key, 'peak', file_path)
            self.compute_d_theta(peak_dominant_strain[key], key, f'peak_{key_dominant}', file_path)
